<template>
  <div>
    ...
    <div class="mb-6">
      ...
      <h1 class="text-3xl font-bold text-gray-900 mt-2">Edit Artikel</h1>
    </div>

    <div v-if="loadingArticle" class="bg-white rounded-lg shadow p-6">
      <p class="text-center text-gray-500">Loading...</p>
    </div>

    <form v-else-if="form.title" @submit.prevent="handleSubmit" class="bg-white rounded-lg shadow p-6">
    ...
    <div v-else class="bg-white rounded-lg shadow p-6">
      <p class="text-center text-gray-500">Artikel tidak ditemukan</p>
    </div>
  </div>
</template>
...
const parseContentForEdit = (content) => {
  if (!content) return ''
  try {
    // Try to parse as JSON array
    const parsed = JSON.parse(content)
    if (Array.isArray(parsed)) {
      // Join array items with double newline for paragraphs
      return parsed.join('\n\n')
    }
  } catch {
  ...
  try {
    ...
    if (response.success) {
      ...
      form.value = {
        ...
        content: parseContentForEdit(article.content),
        image: article.image || '',
        is_highlight: Boolean(article.is_highlight)
      }
    }
  } catch (error) {
